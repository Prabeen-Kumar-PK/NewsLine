import React  from 'react';
import { Link } from "react-router-dom"

const Nav =()=> {
   
        return (
            <div>
                <nav className="navbar fixed-top navbar-expand-lg  " style={{backgroundColor:"#070A52"}}>
                    <div className="container-fluid">
                       <Link className="navbar-brand text-white" to="/home">Newsline</Link>
                        <button className="navbar-toggler bg-light" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
                            <span className="navbar-toggler-icon "></span>
                        </button>
                        <div className="collapse navbar-collapse" id="navbarSupportedContent">
                            <ul className="navbar-nav me-auto mb-2 mb-lg-0">
                                <li className="nav-item text">
                                   <Link className="nav-link text-white" aria-current="page" to="/home">Home</Link>
                                </li>
                                <li className="nav-item text">
                                   <Link className="nav-link text-white" aria-current="page" to="/general">General</Link>
                                </li>
                                <li className="nav-item text">
                                   <Link className="nav-link text-white" aria-current="page" to="/business">Business</Link>
                                </li>
                                <li className="nav-item text">
                                   <Link className="nav-link text-white" aria-current="page" to="/technology">Tech</Link>
                                </li>
                                <li className="nav-item text">
                                   <Link className="nav-link text-white" aria-current="page" to="/sports">Sports</Link>
                                </li>
                                <li className="nav-item text">
                                   <Link className="nav-link text-white" aria-current="page" to="/entertainment">Entertainment</Link>
                                </li>
                                <li className="nav-item text">
                                   <Link className="nav-link text-white" aria-current="page" to="/health">Health</Link>
                                </li>
                                <li className="nav-item text">
                                   <Link className="nav-link text-white" aria-current="page" to="/science">Science</Link>
                                </li>
                            </ul>
                        </div>
                    </div>
                </nav>
            </div>
        )
    }

export default Nav;
